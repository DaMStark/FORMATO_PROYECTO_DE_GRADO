...
{labelfont={bf},labelformat={default},labelsep=newline,name={Figura},justification=centering}

    \newcommand{\figura}[4][width=\textwidth]       % comando figura
    {
        \begin{figure}[!htb]                        % si la fig se mueve usar [h]
            \centering                              % centrado
            \caption{#3}\label{fig:#2}              % titulo de la fig y etiqueta
...
% Título del Tema  
\title{Desarrollo de Sistema periférico no invasivo para monitoreo de desgaste de herramienta
de Corte en tornos CNC industriales en MAINSA.}                         

% Autor
\author{Benjamin Jhasiel Zabalaga Ticona}             

% Tutor             
\newcommand{\tutor}{Nombre del Tutor}     % Si es mujer cambiar \tutor por \tutora (funciona con todo el comité)
...
% Tipo de titulación
\newcommand{\titulacion}{Perfil de Proyecto de Grado}
...
\begin{document}
    ...
    % \input{3_Dedicatorias.tex} % Excluido para presentación de Perfil de Grado
    
        
 % Espaciados de hoja
        \setlength{\parindent}{0pt}     % Sangría
        \setlength{\parskip}{4mm}       % Entre párrafos
        \setstretch{1.4}                % Interlineado CORRECTO
    ...
    % Lista de pseudocódigos (desactivado: no se emplean algoritmos en esta etapa)
    ...
    % Índice de símbolos
        \newpage
        \glsaddall      % Mostrar todos los acrónimos y símbolos definidos
    ...
    \input{1_Seccion_Introduccion.tex}
    \input{2_Seccion_Antecedentes.tex}
    \input{3_Seccion_Planteamiento_Problema.tex}
    \input{4_Seccion_Objetivo_General.tex}
    \input{5_Seccion_Objetivos_Especificos.tex}
    \input{6_Seccion_Alcances_Limites.tex}
    \input{7_Seccion_Justificacion.tex}
    \input{8_Seccion_Fundamento_Teorico.tex}
    %\input{9_Seccion_Propuesta_Metodologica.tex}
    %\input{10_Seccion_Presupuesto.tex}
    %\input{11_Seccion_Cronograma.tex}
